...
const update = async (req, res, next) => {
  try {
    const existing = await pool.query("SELECT * FROM authors WHERE id=$1", [req.params.id]);
    if (!existing.rows[0]) return res.status(404).json({ error: "Muallif topilmadi" });

    const old = existing.rows[0];
    const name = req.body.name ?? old.name;
    const bio = req.body.bio ?? old.bio;
